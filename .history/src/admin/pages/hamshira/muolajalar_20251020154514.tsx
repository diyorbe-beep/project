...
export default function HamshiraMuolajalar() {
  const [treatments] = useState([
    {
      ...
      treatment_name: 'Vena ichiga suyuqlik berish',
      medication: 'Fizioloji eritma',
      dosage: '500ml',
      time: '08:00',
      status: 'Tugallandi',
    ...
    },
    {
      ...
      treatment_name: 'In'ektsiya',
      medication: 'Analgin 2ml',
      dosage: '2ml',
      time: '10:30',
  ...
  const handleStartTreatment = (id: number) => {
    alert(`Muolaja ${id} boshlandi!`);
  };

  const handleCompleteTreatment = (id: number) => {
    alert(`Muolaja ${id} tugallandi!`);
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-gray-600">Bemorlarning muolajalari va dorilarini boshqarish</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 sm:ml-4">
              <p className="text-sm sm:text-base font-bold text-gray-900">
                {treatments.filter(t => t.status === 'Davom etmoqda').length}
              </p>
              <p className="text-xs sm:text-sm font-medium text-gray-600">Davom etmoqda</p>
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dorilar</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {treatments.map((treatment) => (
                <tr key={treatment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 sm:px-6 py-4">
                    <div className="text-xs sm:text-sm text-gray-900">{treatment.medication}</div>
                    <div className="text-xs text-gray-500">{treatment.dosage}</div>
                  </td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div className="text-xs sm:text-sm text-gray-900">{treatment.time}</div>
                  </td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                    <div className="flex space-x-1 sm:space-x-2">
                      ...
                      {treatment.status === 'Davom etmoqda' && (
                        <Button 
                          size="sm" 
              ...
              <div key={treatment.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  <div className="w-8 h-8 bg-pink-100 rounded-full flex items-center justify-center">
                    <i className="ri-user-line text-pink-600 text-sm"></i>
                  </div>
                  <div className="ml-3">
                    <p className="font-medium text-gray-900">{treatment.patient_name}</p>
                    <p className="text-sm text-gray-500">{treatment.treatment_name}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-900">{treatment.time}</p>
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(treatment.status)}`}>
                    {treatment.status}
                  </span>
                </div>
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Muolaja turi</label>
              <select
              ...
              >
                ...
                <option value="In'ektsiya">In'ektsiya</option>
                <option value="Vena ichiga suyuqlik berish">Vena ichiga suyuqlik berish</option>
                <option value="Dori ichish">Dori ichish</option>
                <option value="Mashqlar">Mashqlar</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Dorilar</label>
              <Input
                value={formData.medication}
                onChange={(e) => setFormData({ ...formData, medication: e.target.value })}
                placeholder="Dorilar nomini kiriting"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Doza</label>
              <Input
                value={formData.dosage}
                onChange={(e) => setFormData({ ...formData, dosage: e.target.value })}
                placeholder="Masalan: 2ml, 1 tabletka"
                required
              />
            </div>
          </div>
