...
export default function HamshiralarPage() {
  const [nurses, setNurses] = useState(() => {
    const saved = localStorage.getItem('nurses');
    return saved ? JSON.parse(saved) : nursesData;
  });
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
            <div className="flex items-center">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-yellow-100 rounded-lg flex items-center justify-center">
                <i className="ri-sun-line text-xl sm:text-2xl text-yellow-600"></i>
              </div>
              <div className="ml-3 sm:ml-4">
                <p className="text-xs sm:text-sm font-medium text-gray-600">Kunduzgi smena</p>
                <p className="text-xl sm:text-2xl font-bold text-gray-900">
                  {nurses.filter(nurse => {
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-4 sm:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <h2 className="text-base sm:text-lg font-semibold text-gray-900">Hamshiralar ro'yxati</h2>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-pink-100 rounded-full flex items-center justify-center">
                          <i className="ri-nurse-line text-pink-600 text-sm sm:text-base"></i>
                        </div>
                        <div className="ml-2 sm:ml-4">
                          <div className="text-xs sm:text-sm font-medium text-gray-900">{nurse.fio}</div>
                          <div className="text-xs sm:text-sm text-gray-500">ID: {nurse.id}</div>
                        </div>
                      </div>
                    </td>
